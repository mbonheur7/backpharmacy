from sqlalchemy import (
    Column, Integer, String, Numeric, DateTime, ForeignKey, CheckConstraint, func
)
...
class SaleItem(Base):
    """
    purchase_price/selling_price are snapshotted here at sale time (not
    looked up live from medicines) so a historical receipt stays accurate
    even if the medicine's price changes later.
    """
    ...
    def __repr__(self):
        return f"<SaleItem sale={self.sale_id} medicine={self.medicine_id} qty={self.quantity}>"
