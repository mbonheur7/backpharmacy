    ...
    if expiry_before:
        try:
            query = query.filter(
                ...
                <= parse_date(expiry_before, "expiry_before")
            )
...
def get_medicine(medicine_id):
    ...
    return jsonify({"medicine": serialize_medicine(medicine)}), 200
...
def create_medicine():
    ...
    if not _strict_number(data["purchase_price"]):
        return error_response(
            "purchase_price and selling_price must be numbers."
        )

    if not _strict_number(data["selling_price"]):
        return error_response(
            "purchase_price and selling_price must be numbers."
        )
    ...
    if purchase_price < 0 or selling_price < 0:
        return error_response("Prices cannot be negative.")

    # ----------------------------- quantities

    initial_quantity = data.get("initial_quantity", 0)
    minimum_stock = data.get("minimum_stock", 10)
    critical_stock = data.get("critical_stock", 3)

    if not _strict_integer(initial_quantity):
        return error_response(
            "initial_quantity, minimum_stock, and critical_stock "
            "must be whole numbers."
        )

    if not _strict_integer(minimum_stock):
        return error_response(
            "initial_quantity, minimum_stock, and critical_stock "
            "must be whole numbers."
        )

    if not _strict_integer(critical_stock):
        return error_response(
            "initial_quantity, minimum_stock, and critical_stock "
            "must be whole numbers."
        )
    ...
    medicine = Medicine(
        ...
        medicine_class=(
            (data.get("medicine_class") or "").strip() or None
        ),
        dosage=(
...
def update_medicine(medicine_id):
    ...
    blocked_present = BLOCKED_ON_GENERIC_PATCH & set(data.keys())
...
def update_status(medicine_id):
    ...
    if new_status not in ("Active", "Discontinued"):
        return error_response(
...
def deactivate_medicine(medicine_id):
    ...
    log_activity(
        ...
        entity_type="medicine",
        entity_id=medicine.id,
        details=f"Deactivated {medicine.generic_name}",
    )
...
def reactivate_medicine(medicine_id):
    ...
    log_activity(
        ...
        entity_type="medicine",
        entity_id=medicine.id,
        details=f"Reactivated {medicine.generic_name}",
    )
...
@medicines_bp.post("/<int:medicine_id>/stock-movements")
@login_required
def create_stock_movement(medicine_id):
    ...
    if not _strict_integer(change_qty):
        return error_response(
    ...
    log_activity(
        db_session,
        user,
        "stock_added" if change_qty > 0 else "stock_removed",
        entity_type="medicine",
        entity_id=medicine.id,
        details=(
...
@medicines_bp.get("/<int:medicine_id>/stock-movements")
@login_required
def list_stock_movements(medicine_id):
    ...
    movements = (
        ...
        .order_by(
            desc(StockMovement.occurred_at)
        )
        .all()
    )
